...
vec3 remap(vec3 v, float x1, float x2) {
    return vec3(x1 + v.x * (x2 - x1), x1 + v.y * (x2 - x1), x1 + v.z * (x2 - x1));
}
...
void main()
{
	...
	//color correction
	base = pow(base, vec3(1.0));
	base = clamp(base + vec3(0.0), 0.0, 1.0);
	base *= 1.0;
	base = remap(base,0.0,1.0);
	base = blendOverlay(g2l(facialhair_color.xyz),base);
	//--------------
