...
float limit_value_symmetric(float x) {
    return 40.0 * tanh(x / 20.0);
}
...
void main()
{
    ...
    for( int i = 0; i != au_size; ++i )
    {
        ...
        if(i == 49)
            bs_w = nose_shape_lws.x; // ShapeNoseLength
        if(i == 50)
            bs_w = nose_shape_lws.y; // ShapeNoseWidth
        if(i == 51)
            bs_w = nose_shape_lws.z; // ShapeNoseHeight
        if(i == 52)
            bs_w = eyes_shape_rdum.x; // ShapeEyeSizeVertical  
        if(i == 53)
            bs_w = eyes_shape_rdum.y; // ShapeEyeSizeHorizontal
        if(i == 54)
            bs_w = eyes_shape_rdum.z; // ShapeEyeRoll
        if(i == 55)
            bs_w = eyes_shape_rdum.w; // ShapeEyePositionVertical
        if(i == 56)
            bs_w = eyes_shape_ws.x; // ShapeEyePositionHorizontal
        if(i == 57)
            bs_w = mouth_shape_sv_sh_pv_smile.x; // ShapeMouthSizeVertical
        if(i == 58)
            bs_w = mouth_shape_sv_sh_pv_smile.y; // ShapeMouthSizeHorizontal
        if(i == 59)
            bs_w = mouth_shape_sv_sh_pv_smile.z; // ShapeMouthPositionVertical
        if(i == 60)
            bs_w = face_gender.x; // gender
    ...
    if(attrib_bones[0] == 0u || attrib_bones[0] == 1u)
        mix_weight = attrib_weights[0];
    else if(attrib_bones[1] == 0u || attrib_bones[1] == 1u)
        mix_weight = attrib_weights[1];
    else if(attrib_bones[2] == 0u || attrib_bones[2] == 1u)
        mix_weight = attrib_weights[2];
    else if(attrib_bones[3] == 0u || attrib_bones[3] == 1u)
        mix_weight = attrib_weights[3];
    ...
    mv[0] = mix( mv[0], vec4(0.1 *scale,0.,0.,0.), mix_weight);
    mv[1] = mix( mv[1], vec4(0.,0.,-0.1*scale,0.), mix_weight);
    mv[2] = mix( mv[2], vec4(0.,0.1*scale,0.,0.), mix_weight);
