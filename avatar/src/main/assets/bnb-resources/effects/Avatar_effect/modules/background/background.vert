...
{
    vec2 fg_tex_size = vec2(textureSize(BNB_SAMPLER_2D(tex_background), 0));
    ...
    float content_mode = background_aspect_rotation.x;
...
void main()
{
    ...
    var_uv = v * 0.5 + 0.5;
    var_uv += mix(face_rotation.xy /5000., vec2(0.0), is_face_anim.y);

    var_uv2 = aspect_fill_uv_tex(var_uv);
    var_uv2 = bnb_rotate_uv(var_uv2, background_aspect_rotation.y * 3.14/180.);
