...
void main()
{
    ...
    for( int i = 0; i != au_size; ++i )
    {
        ...
        if(i == 49)
            bs_w = brows_shape_tcar.x; // ShapeBrowThick
        if(i == 50)
            bs_w = brows_shape_tcar.y; // ShapeBrowClassic
        if(i == 51)
            bs_w = brows_shape_tcar.z; // ShapeBrowAngular
        if(i == 52)
            bs_w = brows_shape_tcar.w; // ShapeBrowRounded
        if(i == 53)
            bs_w = brows_shape_sh_pv.x; // ShapeBrowSizeHorizontal
        if(i == 54)
            bs_w = brows_shape_sh_pv.y; // ShapeBrowPositionVertical 
        if(i == 55)
